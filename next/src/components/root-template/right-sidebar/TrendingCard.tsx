...
export default function TrendingCard({ hashtag }: { hashtag: TrendingHashtagsType }) {
    ...
    return (
        <Link href={`http://localhost:3000/search?q=${hashtag.name}`} className='p-2 rounded flex flex-col text-14 text-secondary-text hover:bg-card-hover hover:cursor-pointer'>
            <p>Trending</p>
