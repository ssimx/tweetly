...
import { getErrorMessage, UserDataType, AppError, ErrorResponse, SuccessResponse } from 'tweetly-shared';
...
export async function GET(req: NextRequest, props: { params: Promise<{ username: string }> }) {
    if (req.method === 'GET') {
        ...
        try {
            ...
            if (cursor !== null) {
                const response = await fetch(`${apiUrl}/users/followers/${params.username}?cursor=${cursor}`, {
                    method: 'GET',
                    headers: {
                ...
                else if (!data.followers) throw new AppError('Followers property is missing in data response', 404, 'MISSING_PROPERTY');
                ...
                const response = await fetch(`${apiUrl}/users/followers/${params.username}`, {
                    method: 'GET',
                    headers: {
                        'Content-Type': 'application/json',
                        'Authorization': `Bearer ${token}`,
