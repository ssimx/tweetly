...
export async function GET(req: NextRequest, props: { params: Promise<{ username: string }> }) {
    if (req.method === 'GET') {
        ...
        try {
            ...
            const { data } = await response.json() as SuccessResponse<{ user: UserDataType }>;
            if (!data) throw new AppError('Data is missing in response', 404, 'MISSING_DATA');
            else if (!data.user) throw new AppError('User profile data is missing in data response', 404, 'MISSING_PROFILE_DATA');
            ...
            }) as NextResponse<SuccessResponse<{ user: UserDataType }>>
        } catch (error: unknown) {
