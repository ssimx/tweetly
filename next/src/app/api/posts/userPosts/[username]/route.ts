...
export async function GET(req: NextRequest, props: { params: Promise<{ username: string }> }) {
    if (req.method === 'GET') {
        ...
        try {
            ...
            if (cursor !== null) {
                const response = await fetch(`${apiUrl}/posts/${params.username}?cursor=${cursor}`, {
                    method: 'GET',
                    headers: {
                ...
                else if (!data.posts) throw new AppError('Posts property is missing in data response', 404, 'MISSING_PROPERTY');
