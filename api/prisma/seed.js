...
function getRandomHashtags() {
  ...
  if (selectedHashtags.length < 5) {
    for (
      let i = selectedHashtags.length;
      i < Math.random() * (5 - selectedHashtags.length);
      i++
    ) {
...
function getRandomUsernameLength() {
  ...
  const totalWeight = lengthOptions.reduce(
...
async function main() {
  ...
  const users = await Promise.all(
    Array.from({ length: 100 }).map(async (v, index) => {
      ...
      const usernameAndName = generateUsername(
        '',
        3,
        randomizedUsernameLength
      ).replaceAll('-', '');
  ...
  for (const user of users) {
    ...
    for (const followee of followees) {
      if (user.id !== followee.id) {
        ...
        if (
          followCreatedAt > user.createdAt &&
          followCreatedAt > followee.createdAt
        ) {
  ...
  for (let i = 0; i < 500; i++) {
    ...
    const hashtagsContent = Array.from(
      new Set(postContent.match(hashtagRegex)?.map((tag) => tag.slice(1)) || [])
    );
  ...
  for (let i = 0; i < 1000; i++) {
    ...
    const hashtagsContent = Array.from(
      new Set(
        replyContent.match(hashtagRegex)?.map((tag) => tag.slice(1)) || []
      )
    );
  ...
  for (const post of randomPosts) {
    ...
    for (const user of randomUsers) {
      ...
      if (shouldRepost) {
        const notificationReceiversForReposter =
          await prisma.pushNotification.findMany({
      ...
      // Create random likes
      const shouldLike = Math.random() < 0.5; // 50% chance to like
      if (shouldLike) {
  ...
  for (const [index, user] of allUsers.entries()) {
    ...
    const receivers = mutualFollows.map((follow) =>
      follow.followerId === user.id
        ? { id: follow.followeeId, createdAt: follow.createdAt }
        : { id: follow.followerId, createdAt: follow.createdAt }
    );
    const messagePartners = faker.helpers.arrayElements(
      receivers,
      faker.number.int({ min: receivers.length / 2, max: receivers.length })
    );
      ...
      const conversationExists = await prisma.conversation.findFirst({
        where: {
          participants: {
            ...
            every: {
              OR: [{ userId: user.id }, { userId: receiver.id }],
            },
          },
        },
        select: {
          ...
          newMessageTime = faker.date.between({
            ...
            to: new Date(
              lastMessageTime.getTime() +
                faker.number.int({ min: 60 * 1000, max: 6 * 60 * 60 * 1000 })
            ),
          });
